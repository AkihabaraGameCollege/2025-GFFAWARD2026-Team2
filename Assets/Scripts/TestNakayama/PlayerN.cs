...
[RequireComponent(typeof(CharacterController))]// CharacterController コンポーネントを必須にする（ChatGPTから引用By中山）
...
public class PlayerN : MonoBehaviour
{
    ...
    */
    ...
    //（ChatGPTから引用By中山）
    [Header("References")]
    public Transform cameraTransform;      // カメラの Transform（Inspectorで割当て。未割当てなら Start で Camera.main を使う）

    [Header("Movement")]
    public float speed = 6f;               // 移動速度
    public float rotationSpeed = 10f;      // 回転の速さ（大きいほど素早く向く）

    [Header("Physics")]
    public float gravity = -9.81f;         // 重力（負の値）
    public float groundedGravity = -2f;    // 地面にいるときの微小下方向力（0にしないと浮くことがある）

    CharacterController controller;
    Vector3 velocity;                      // Y方向の速度（重力処理用）
    ...
    private void Start()
    {
        ...
        // カメラがInspectorでセットされていなければ Main Camera を使う
    ...
    void Update()
    {
        ...
        {
            // カメラの前方向と右方向を取得し、Y成分を0にする（水平成分だけ使う）
            ...
            // 3) プレイヤーを移動（CharacterController を使う）
